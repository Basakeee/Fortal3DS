...
- `pigGenerator.js` / `dogGenerator.js` / `cowGenerator.js` / `catGenerator.js` / `chickenGenerator.js` — the 5 farm animals `MemoryFarmGameManager.cs`'s `animalTypes` currently only distinguishes by tinting one flat placeholder sprite (see its own `NamedColor[]`); each generator's default color matches that file's `Color(...)` value exactly, same "reads as the same item" reasoning `gemGenerator.js`'s `GEM_PRESETS` gives for Boss Slayer's pickups. Unlike the boss presets (one dominant block, deliberately no limbs), these stand on 4 short leg columns — same corner-column technique a car preset's wheels use, boxes instead of discs — since 4-legged silhouette is itself load-bearing for "farm animal" at this resolution. Species differ mainly in proportion/color/one or two distinguishing features rather than a shared body kit: pig gets a flat wide snout, dog a narrow projecting snout + down-hanging ears (vs. pig's up-pointing ear nubs), cow explicit hand-placed coat patches (a plain-colored blob doesn't read as "cow" the way it still reads as "pig"/"cat"), cat up-pointed tapered ears + the longest tail of the 4. `chickenGenerator.js` is the one biped and the only farm animal with a rounded (ellipsoid distance-test, not boxy) body, plus comb/beak/wattle/wings — 2 legs alone is a strong enough "bird" cue that it skips every other animal's tail/ear treatment.
